...
if __name__ == '__main__':
    ...
    # print(data)
    # pic=cv2.imread("wangqiu.png")

    test=[]
    counter = 0
    stage = None
    # dir_path="D:\\science\\Tennis_pose\\images"

    # files = os.listdir(dir_path)

    # 打开视频文件
    cap = cv2.VideoCapture('E:/MyTennis/Tennis_System/input_videos/player.mp4')
    ...
    out = cv2.VideoWriter('E:/MyTennis/Tennis_System/output_videos/pose_output.avi', fourcc, fps, (width, height))
    ...
    while cap.isOpened():
        ...
        with mp_pose.Pose(min_detection_confidence=0.3, min_tracking_confidence=0.8) as pose:
            ...
            try:
                ...
                test_2d = np.array(test).reshape(1, -1)
                temp=svm_test.svm_sort(test_2d)
            ...
            cv2.rectangle(image, (0, 0), (700, 40), (235, 30, 3), -1)
            cv2.putText(image, temp, (12, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    out.release()
    #data = pd.DataFrame(data=test)  # 将数据放进表格
    # data.to_csv('standard_data.csv',mode='a',index=False)